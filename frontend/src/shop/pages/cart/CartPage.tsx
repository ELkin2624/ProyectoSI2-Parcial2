...
export const CartPage = () => {
    ...
    const { data, updateItemMutation, deleteItemMutation } = useCart();
                                        ...
                                        {data.items.map((item) => (
                                            <div key={item.id} className="flex gap-6 p-6 border border-border bg-card">
                                                ...
                                                <div className="relative h-32 w-24 shrink-0 bg-muted overflow-hidden">
                                                    <img src={item.variante.imagen_url || "/placeholder.svg"} alt={item.variante.nombre} className="object-cover" />
                                                </div>

                                                {/* Product Details */}
                                                <div className="flex-1 flex flex-col justify-between">
                                                    <div>
                                                        <h3 className="text-lg font-light">{item.variante.nombre}</h3>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex items-center gap-2">
                                                            <button
                                                                ...
                                                                className="h-8 w-8 border border-border hover:border-foreground transition-colors flex items-center justify-center disabled:opacity-50"
                                                            >
                                                                <Minus className="h-4 w-4" />
                                                            </button>
                                                            <span className="text-sm font-light w-8 text-center">{item.cantidad}</span>
                                                            <button
                                                                onClick={() => handleUpdateQuantity(item.id, item.cantidad + 1)}
                                                                disabled={updateItemMutation.isPending}
                                                                className="h-8 w-8 border border-border hover:border-foreground transition-colors flex items-center justify-center disabled:opacity-50"
                                                ...
                                                <div className="flex flex-col items-end justify-between">
                                                    <p className="text-lg font-light">${item.subtotal}</p>
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
                                ...
                                <div className="lg:col-span-1">
                                    <div className="sticky top-24 border border-border p-8 space-y-6">
                                                    ...
                                                    data.items.map(item => (
                                                        <div key={item.id} className="flex justify-between">
                                                            <span className="text-muted-foreground">
                                                                {item.variante.nombre} ({item.cantidad} {item.cantidad === 1 ? "artículo" : "artículos"})
                                                            </span>
                                                            <span className="font-light">${item.subtotal}</span>
                                                        </div>
                                                    ))
                                                }
                                            </div>
                                        ...
                                        <div className="flex justify-between text-xl">
                                            ...
                                            <p className="text-lg font-light">${data.total_carrito}</p>
                                        </div>
